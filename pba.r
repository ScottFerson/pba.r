...
cut        <- function(x,s,...) {UseMethod("cut")}
...
#cat(currentTransformedMean,'\n')
#cat(transformedMean_lower,'\n')
...
perfectconv.pbox <- function(a,b,op='+') {
  protectedlog <- function(a) return(log(pmax(a,0.00001)))
...
MLlognormal <- function(x) {n=length(x); mu=sum(log(x))/n; sig2=sum((log(x)-mu)^2)/n; return(lognormal(mean=exp(mu+(sig2)/2), std=sqrt((exp(sig2)-1)*exp(2*mu+sig2))))}
...
CBbernoulli.p <- function(x) {n <- length(x); k <- sum(x); return(env(beta(k, n-k+1), beta(k+1, n-k)))}

# x[i] ~ binomial(N, p), for known N, x[i] is a nonnegative integer less than or equal to N

CBbinomial <- function(x,N) {n <- length(x); k<-sum(x); return(env(betabinomial(N,k,n*N-k+1),betabinomial(N,k+1, n*N-k)))}
CBbinomial.p <- function(x,N) {n <- length(x); k <- sum(x); return(env(beta(k, n*N-k+1), beta(k+1, n*N-k)))}

# x[i] ~ binomial(N, p), for unknown N, x[i] is a nonnegative integer

# see https://sites.google.com/site/cboxbinomialnp/
CBbinomialnp <- function(x) {stop('see https://sites.google.com/site/cboxbinomialnp/')}
...
CBpoisson.mean <- function(x) {n <- length(x); k = sum(x); return(env(gamma(shape=k, rate=n),gamma(shape=k+1, rate=n)))}

# x[i] ~ exponential(mean), x[i] is a nonnegative integer

CBexponential <- function(x) {n <- length(x); k = sum(x); return(gammaexponential(shape=n, rate=k))}
CBexponential.mean <- function(x) {n <- length(x); k = sum(x); return(gamma(shape=n, rate=k))}
CBexponential.lambda <- function(x) {return(1/CBexponential.mean(x))}

# x[i] ~ normal(mu, sigma)

CBnormal <- function(x) {n <- length(x); return(mean(x) + sd(x) * student(n - 1) * sqrt(1 + 1 / n))}
...
CBlognormal <- function(x) {n <- length(x); return(exp(mean(log(x)) + sd(log(x)) * student(n - 1) * sqrt(1+1/n)))}
...
#CBuniform <- function(x) {n=length(x); w=(max(x)-min(x))/beta(length(x)-1,2); m=(max(x)-w/2)+(w-(max(x)-min(x)))*uniform(0,1); return(uniform(m-w/2, m+w/2))}
CBuniform <- function(x) {n=length(x); r=max(x)-min(x); w=(r/rbeta(1e4,length(x)-1,2))/2; m=(max(x)-w)+(2*w-r)*runif(1e4,0,1); return(histogram(runif(1e4,m-w, m+w),conf=0))}
...
CBnonparametric <- function(x) return(env(histogram(c(x, Inf), conf=0), histogram(c(x, -Inf), conf=0)))

# x1[i] ~ normal(mu1, sigma1), x2[j] ~ normal(mu2, sigma2), x1 and x2 are independent

CBnormal.meandifference <- function(x1, x2) CB.normal.mu(x2) - CB.normal.mu(x1)

# x[i] = Y + error[i],  error[j] ~ F,  F unknown,  Y fixed,  x[i] and error[j] are independent

CBnonparametric.deconvolution <- function(x, error) {# i.e., the c-box for Y
    z = NULL
